...
class Predictor(nn.Module):
    ...
    def __init__(self, num_classes:list[int] = [2], regressions:int = 1, latent_dim=32):
        ...
        self.regressions = nn.ModuleList()
        for _ in range(regressions):
            self.regressions.append(Regression(3, 64, 64))
    ...
    def train_concepts(self, x, classifications, regressors):
        ...
        for j, regression in enumerate(self.regressions):
            regression_loss = regression.calculate_loss(latent, regressors[:, j])
            concept_loss += regression_loss

        concept_loss = concept_loss/(i+j+2)
